...
GREEN='\033[0;32m'
NC='\033[0m'
items=( "kafka" "kafka-connect" "custom-kafka-file-connector" "custom-kafka-CPU-connector" "custom-kafka-chunk-correlation-connector")
...
function create_connector() {
    ...
    temp_file=$(mktemp)
    count_of_connector=$(curl -s GET http://localhost:8083/connectors/ | jq .[] | wc -l)
    Increment_value=$(( (count_of_connector + 1) * 10 ))
    #echo $a
    sed -e "s/{{CONNECTOR_NAME}}/$cname/g" -e "s/{{TABLE_NAME}}/$tname/g" -e "s/{{date_time}}/$date_time/g" -e "s/{{Incr_value}}/$Increment_value/g" "$TEMPLATE_PATH" > "$temp_file"
...
function check_topic_data_live() {
    read -p "Enter topic name to consume LIVE DATA: " tname
    echo "Press Ctrl+C to stop consuming."
    $KAFKA_BIN_PATH/kafka-console-consumer.sh --bootstrap-server $BOOTSTRAP_SERVER --topic "$tname"
}
...
function display_retention_for_topics(){
        read -p "Enter topic name(s) to display (comma-separated for multiple) retention: " tnames
        IFS=',' read -ra TOPIC_ARRAY <<< "$tnames"

        for topic in "${TOPIC_ARRAY[@]}";do
                echo -e "$topic Retention Policy status"
                $KAFKA_BIN_PATH/kafka-configs.sh --bootstrap-server $BOOTSTRAP_SERVER --entity-type topics --entity-name $topic  --describe
        done
}
...
while true; do
    ...
    echo "===================================="
    echo -e "\n\t1. Check the status of Kafka,kafka-connect,Custom-file-connector service,CPU-Service,Correlated-SNMP-data"
    ...
    echo -e "\n\t13. Delete a Topic"
    echo -e "\n\t14. Display the log retention policy for particular Topics"
    echo -e "\n\t15. Display the LIVE DATA Topic Only"
    echo -e "\n\t16. Exit"${NC}
    ...
    case $choice in
        ...
        12) delete_connector ;;
        13) delete_topic ;;
        14) display_retention_for_topics;;
        15) check_topic_data_live;;
